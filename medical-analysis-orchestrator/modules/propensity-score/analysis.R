...
run_module <- function(config, context) {
  ...
  plot_overlap<-function(){palette<-medical_figure_palette();control_color<-grDevices::adjustcolor(palette[["neutral"]],.42);treated_color<-grDevices::adjustcolor(palette[["accent"]],.42);graphics::hist(ps[data$.treated==0],breaks=20,freq=FALSE,col=control_color,border=NA,xlim=c(0,1),xlab="倾向评分",main="倾向评分重叠");graphics::hist(ps[data$.treated==1],breaks=20,freq=FALSE,col=treated_color,border=NA,add=TRUE);graphics::legend("top",legend=c("未处理","处理"),fill=c(control_color,treated_color),bty="n")}
